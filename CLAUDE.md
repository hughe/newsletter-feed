...
- **`fetch(request, env, ctx)`** — a hand-rolled path router (no framework). The RSS feed lives at `/feed/<token>.xml`; admin pages live under `/feed/<token>/...`. HTML pages are built by string-templating functions (`layout`, `renderDashboard`, `renderEmailList`, `renderEmailView`, etc.) sharing the `STYLE` constant. `/feed/<token>/email/<id>/view` (`renderEmailRaw`) is a chrome-free version of a single email — the stored HTML served directly as the page (no nav, no iframe), sandboxed via a `Content-Security-Policy: sandbox` header, with a small "fit to screen" stylesheet (`FIT_CSS` via `injectMobileFit`) injected so non-responsive fixed-width newsletters shrink to fit mobile Safari — and is the URL used in each RSS item's `<link>` (so "open in browser" in a reader shows the message rendered faithfully, not the reader's mangled CSS). `POST /feed/<token>/error/<id>/replay` (and a button on the error view) re-runs a stored failure through the parser — see replay below.
...
- **Stored HTML is untrusted third-party content.** The single-email admin view renders it in an `<iframe sandbox>` (no scripts, no same-origin); don't loosen that sandbox. The chrome-free `/view` endpoint instead serves the HTML *directly* as the page (no iframe — iframes render poorly in mobile Safari) and applies the equivalent protection with a `Content-Security-Policy: sandbox` response header (no scripts run; the page gets a unique opaque origin, which matters because the URL carries the token). Don't drop that CSP header. `/view` also sends `Referrer-Policy: no-referrer` so the token-bearing URL isn't leaked to the newsletter's remote assets. In the RSS feed it goes in `content:encoded` CDATA — the RSS client is responsible for its own rendering safety. HTML is intentionally **not** sanitized (newsletters already carry tracking pixels/remote assets; this matches opening them in a mail client).
