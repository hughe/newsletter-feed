...
- **`fetch(request, env, ctx)`** — a hand-rolled path router (no framework). The RSS feed lives at `/feed/<token>.xml`; admin pages live under `/feed/<token>/...`. HTML pages are built by string-templating functions (`layout`, `renderDashboard`, `renderEmailList`, `renderEmailView`, etc.) sharing the `STYLE` constant. `/feed/<token>/email/<id>/view` (`renderEmailRaw`) is a chrome-free version of a single email — just the sandboxed iframe, full-bleed, no nav — and is the URL used in each RSS item's `<link>` (so "open in browser" in a reader shows the message rendered faithfully, not the reader's mangled CSS). `POST /feed/<token>/error/<id>/replay` (and a button on the error view) re-runs a stored failure through the parser — see replay below.
...
- **Stored HTML is untrusted third-party content.** Both the single-email admin view and the chrome-free `/view` endpoint render it in an `<iframe sandbox>` (no scripts, no same-origin). Don't loosen the sandbox. The `/view` page also sends `Referrer-Policy: no-referrer` so the token-bearing URL isn't leaked to the newsletter's remote assets. In the RSS feed it goes in `content:encoded` CDATA — the RSS client is responsible for its own rendering safety. HTML is intentionally **not** sanitized (newsletters already carry tracking pixels/remote assets; this matches opening them in a mail client).
